...
<a href="https://youtube.com/shorts/h959soKaR9U"><img src="https://img.youtube.com/vi/h959soKaR9U/0.jpg" width="320" alt="▶ 플레이 테스트 영상 보기"></a>
...
| 감정 분석 | OpenAI GPT-4o-mini — 5종 감정 분류 + 30자 내외 공감 응답 생성 + 중복 판단 (30초 타임아웃) |
| 대화 기록 저장 | STT 텍스트 + AI 응답을 CSV로 자동 저장, 기존 발화와 유사하면 저장 스킵 |
...
| 레이어 | 역할 | 주요 클래스 |
|--------|------|------------|
| **AI** | 외부 API 직접 호출 (STT→감정분석→TTS) + 녹음/재생 + 대화 기록 저장 | `AIService`, `VoiceEmotionAnalyzer`, `VoiceProcessResponse` |
...
├── Scripts/
│   ├── AI/
│   │   ├── AIService.cs                # 외부 API 직접 호출 (Clova STT, OpenAI, Clova TTS) + 대화 기록 CSV 저장
...
└── StreamingAssets/
    ...
    ├── conversation_log.csv           # 대화 기록 (STT 텍스트, AI 응답, 감정코드)
...
3. 버튼 땜 (data[1]==0) → StopRecordingAndProcess()
   ...
   Clova STT → GPT-4o-mini 감정분석(+중복 판단) → 비중복 시 CSV 저장 → Clova TTS
      ↓
...
| 날짜 | 내용 |
|------|------|
| 2026-03-31 | 대화 기록 CSV 저장 + OpenAI 기반 유사 발화 중복 판단 기능 추가 |
